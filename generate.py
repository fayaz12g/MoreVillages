...
def reskin_nbt(path, out_path, replacements):
    nbt = load(path)
    palette = nbt.root.get("palette", [])
    for block in palette:
        name = block["Name"]
        if name in replacements:
            block["Name"] = replacements[name]
    File(nbt.root).save(out_path)
...
def reskin_villages(base_dir, out_dir):
    for theme, mats in VILLAGE_THEMES.items():
        ...
        # if stone family doesn’t exist (like "sandstone" or custom "palm"), skip gracefully
